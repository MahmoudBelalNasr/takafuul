...
@section('title')
    تكافل للرعاية الصحية | تعديل الصور
@endsection
...
@section('content')
    <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-4">
            <h4 class="font-weight-bolder"><span class="badge badge-pill badge-danger">تعديل صورة</span> </h4>
        </div>
                                ...
                                <div class="card-body" id="print">
                                    <form action="{{ route('image.update', $image->id) }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            <label for="name">تحميل صورة:</label>
                                            <input type="file" name="upload_img" class="form-control">
                                            @error('upload_img')
                                            <small class="form-text text-danger">{{$message}}</small>
                                            @enderror
                                        </div>
                                        <img src="{{ asset('/uploads/images_library/'.$image->image) }}" width="100px"><br><br>
                                        <button type="submit" class="btn btn-primary">اضافة</button>
                                        <a href="{{ route('image.index') }}" class="btn btn-danger">إلغاء</a>
                                    </form>
                                </div>
                            </div>
                        </div>
